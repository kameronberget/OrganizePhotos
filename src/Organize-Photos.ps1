...
function Organize-Pictures {
    ...
    switch ($Scope) {
        ...
        "SortCurrentDirectory" {
            $pictures = if ($Recurse) { Get-ChildItem -Path * -Recurse -Include *.jpeg, *.png, *.gif, *.jpg, *.bmp, *.png } else { Get-ChildItem  }
            break;
        }

        "SortTargetDirectory" {
            $pictures = if ($Recurse) { Get-ChildItem -Path $target -Recurse -Get-ChildItem -Path * -Recurse -Include *.jpeg, *.png, *.gif, *.jpg, *.bmp, *.png} else { Get-ChildItem $target  }
            break;
        }
